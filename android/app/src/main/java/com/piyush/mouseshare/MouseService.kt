...
class MouseService : AccessibilityService() {
    ...
    companion object {
        ...
        @Volatile var rawStatus: String = "real keys: helper not started"

    }

    private val main = Handler(Looper.getMainLooper())

    /** Connection to the shell helper that injects real key events (see KeyServer). */
    private val helper = HelperLink().also {
        it.onLost = {
            rawStatus = "real keys OFF: helper stopped (start it again from the PC app)"
            send("RAWKEYS 0 helper stopped")
        }
    }
    ...
    // ---- real mouse (through the shell helper) - see KeyServer
    @Volatile private var realMouseWanted = false      // PC setting, sent when input moves to the phone
    @Volatile private var rightClickBack = false       // also send Back on right-click
    private var sentX = -1f                            // last position handed to the helper
    private var sentY = -1f

    /** True when mouse input should go to Android as real mouse events instead of touch gestures. */
    private fun realMouse() = realMouseWanted && helper.isUp
    ...
    override fun onServiceConnected() {
        ...
        readScreenSize()
        switcher = AppSwitcher(this).also { sw ->
            try { rootInActiveWindow?.packageName?.toString()?.let { sw.onWindowEvent(it) } }
            catch (e: Exception) {}
        }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        // remember which app is on screen, for the Alt+Tab list
        if (event != null && event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            val pkg = event.packageName?.toString() ?: return
            switcher?.onWindowEvent(pkg)
        }
    }

    /** For the setup screen: how the mouse reaches Android right now. */
    fun mouseMode(): String =
        if (realMouse()) "real mouse events (wheel, hover, buttons)"
        else if (helper.isUp) "touch gestures (the PC has real keys/mouse switched off)"
        else "touch gestures - start the real-key helper for a real mouse"

    /** For the setup screen: the apps Alt+Tab knows about right now. */
    fun switcherSummary(): String = switcher?.summary() ?: "-"
    ...
    private fun handleClient(s: Socket) {
        try {
            ...
            send("OK")
            send(infoLine())
            if (helper.isUp) send("RAWKEYS 1 ${helper.info}")
    ...
    private fun handleLine(line: String) {
        ...
        try {
            when (p[0]) {
                ...
                "S" -> {
                    val n = p[1].toFloat()
                    // real wheel event at the pointer; otherwise a swipe gesture
                    if (realMouse()) helper.send(MouseLines.wheel(0f, n))
                    else main.post { scrollY += n; pump() }
                }
                "H" -> {
                    val n = p[1].toFloat()
                    if (realMouse()) helper.send(MouseLines.wheel(n, 0f))
                    else main.post { scrollX += n; pump() }
                }
                "REALMOUSE" -> {
                    realMouseWanted = p.getOrNull(1) == "1"
                    rightClickBack = p.getOrNull(2) == "1"
                    main.post { sentX = -1f }
                }
                ...
                "KEYSRV" -> connectHelper(p[1])
                "KD", "KU" -> forwardRaw(line.trim())          // real key events -> shell helper
                "RAWRESET" -> helper.send("RESET")
    ...
    /** Tell the helper where the pointer is (a real hover move). Only when it changed. */
    private fun sendHover() {
        if (!active || !realMouse()) return
        if (curX == sentX && curY == sentY) return
        sentX = curX
        sentY = curY
        helper.send(MouseLines.hover(curX, curY))
    }
    ...
    private fun setActive(on: Boolean) {
        ...
        if (!on) {
            altTabCancel()
            helper.send("RESET")             // never leave Alt/Ctrl held down on the phone
        }
        if (on) {
            syncPos()
            attachCursor()
    ...
    /** The PC started the helper over adb and gave us its token: connect and tell the PC how it went. */
    private fun connectHelper(token: String) {
        thread(name = "mouseshare-helper-connect", isDaemon = true) {
            val err = helper.connect(token)
            if (err == null) {
                rawStatus = "real keys ON (${helper.info})"
                send("RAWKEYS 1 ${helper.info}")
            } else {
                rawStatus = "real keys OFF: $err"
                send("RAWKEYS 0 $err")
            }
        }
    }

    private fun forwardRaw(line: String) {
        if (!helper.send(line) && rawStatus.startsWith("real keys ON")) {
            rawStatus = "real keys OFF: lost the helper"
            send("RAWKEYS 0 lost the helper")
        }
    }
    ...
    private fun navigate(what: String) {
        ...
        when (what) {
            "ALTTAB" -> altTab(1)
            "ALTTABBACK" -> altTab(-1)
            "ALTTABEND" -> altTabFinish()
            else -> {
                altTabCancel()                     // any other system key abandons the switcher
                when (what) {
    ...
    // ---- Alt+Tab: our own switcher strip (see AppSwitcher)
    private var switcher: AppSwitcher? = null
    private var altTabSystem = false        // not enough apps known yet: this Alt press used plain Recents

    private fun altTab(dir: Int) {
        if (altTabSystem) return             // Recents is already open for this Alt press
        val sw = switcher
        if (sw == null || !sw.step(dir)) {
            altTabSystem = true
            performGlobalAction(GLOBAL_ACTION_RECENTS)
            KeyInput.lastResult = "Alt+Tab: no other app known yet, opened Recents " +
                "(open a couple of apps on the phone and it will switch between them)"
        }
    }

    private fun altTabFinish() {
        if (altTabSystem) {
            altTabSystem = false
            return
        }
    ...
    private fun onButton(name: String, down: Boolean) {
        if (down) altTabCancel()                     // clicking abandons a half-finished Alt+Tab
        if (realMouse()) {
            realButton(name, down)
            return
        }
        when (name) {
            "L" -> {
    ...
    /**
     * A button as a real mouse button: left/right/middle become mouse buttons, the two side buttons
     * become Back / Forward. No touch gesture is involved, so no system edge gesture can trigger.
     */
    private fun realButton(name: String, down: Boolean) {
        if (name == "L" || name == "R" || name == "M") {
            sentX = -1f
            sendHover()                              // the click must land where the cursor is
        }
        for (line in MouseLines.button(name, down, rightClickBack)) helper.send(line)
    }
    ...
    private fun dispatchScroll() {
        ...
        // Android treats a swipe that starts near the screen edge as a system gesture (pull-down
        // shade from the top, home / recents from the bottom, back from the sides). Keep the whole
        // swipe inside a safe margin by sliding it, not by squashing it.
        val safe = 56f * resources.displayMetrics.density
        val cx = safeCenter(curX, abs(dx) / 2, safe, screenW.toFloat())
        val cy = safeCenter(curY, abs(dy) / 2, safe, screenH.toFloat())
        val x0 = cx - dx / 2
        val x1 = cx + dx / 2
        val y0 = cy - dy / 2
        val y1 = cy + dy / 2
    ...
    /** Move a swipe's centre so that both ends stay at least [margin] px away from the screen edges. */
    private fun safeCenter(c: Float, half: Float, margin: Float, size: Float): Float {
        val lo = margin + half
        val hi = size - margin - half
        return if (lo > hi) size / 2 else c.coerceIn(lo, hi)
    }
