...
Wire protocol (UTF-8 text, one command per line, TCP port 5599):
    PC -> phone   HELLO <pin>           first line, must match the PIN on the phone
                  ...
                  KD <keycode> [1]      REAL key down (Android keycode; 1 = auto-repeat)  } only when the
                  KU <keycode>          REAL key up                                       } phone reported
                  RAWRESET              release every real key still held                 } RAWKEYS 1
                  REALMOUSE <0|1> <0|1> use real mouse events (helper running + enabled), and also
                                        send Back on right-click; sent each time input moves to the phone
                  KEYSRV <token>        connect to the shell helper the PC just started over adb
                  NAV <BACK|HOME|RECENTS|NOTIFICATIONS>
                  NAV ALTTAB / ALTTABBACK / ALTTABEND
                                        app switcher session: ALTTAB opens Recents the first time
                                        and slides to the next app after that, ALTTABBACK slides
                                        the other way, ALTTABEND (Alt released) opens the
                                        highlighted app
    ...
    phone -> PC   OK / NO               answer to HELLO
                  ...
                  RAWKEYS 1 <how> | RAWKEYS 0 <why>   real-key helper is / is not usable
                  PONG
Discovery (UDP 5598):
...
DEFAULTS = {
    ...
    "real_keys": True,            # use real key + mouse events (real Alt+Tab, real wheel...) when the phone helper runs
    "right_click_back": False,    # real-mouse mode: right-click ALSO sends Back (tick if your phone does not do it itself)
    # --- feel ---
    "sensitivity": 1.0,
    "scroll_speed": 1.0,
...
class Bridge:
    def __init__(self, cfg, screen):
        ...
        self.raw_keys = False            # phone says its shell helper can inject real key events
        self.raw_info = ""               # ...how, or why not
    ...
    def _reader(self, s, f):
        try:
            while True:
                ...
                elif len(parts) >= 2 and parts[0] == "RAWKEYS":
                    self.raw_keys = parts[1] == "1"
                    self.raw_info = " ".join(parts[2:])
                    log("real keys %s %s" % ("ON" if self.raw_keys else "OFF", self.raw_info))
                    if self.on_change:
                        self.on_change()
        except (OSError, ValueError):
            pass
    ...
    def enter_remote(self, frac=None):
        ...
        with self.lock:
            ...
            self.remote = True
            self._edge_since = None
            # tell the phone how to treat the mouse BEFORE the first position arrives
            self.send("REALMOUSE %d %d" % (1 if self.raw_active() else 0,
                                           1 if self.cfg["right_click_back"] else 0))
            self._send_pos()
            self.send("ACTIVE 1")
    ...
    def exit_remote(self, frac=None):
        ...
        with self.lock:
            ...
            self.keys_down.clear()
            if self.raw_keys:
                self.send("RAWRESET")         # never leave Alt/Ctrl held down on the phone
            self.send("ACTIVE 0")
            l, t, r, b = self.screen.virtual_rect()
            if frac is not None:
    ...
    def raw_active(self):
        """True when keys should go to the phone as real key events (helper running + enabled)."""
        return self.raw_keys and bool(self.cfg["real_keys"])

    def send_key_down(self, keycode, repeat=False):
        self.send("KD %d %d" % (int(keycode), 1 if repeat else 0))

    def send_key_up(self, keycode):
        self.send("KU %d" % int(keycode))

    def send_keysrv(self, token):
        self.send("KEYSRV " + token)
