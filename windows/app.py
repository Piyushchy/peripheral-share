...
class App:
    ...
    def _build(self):
        ...
        ttk.Checkbutton(box, text="System keys act on the device the cursor is on "
                                  "(Win / Win+D = Home, Win+Tab = Recents, Alt+Tab = app switcher, Win+A = Notifications, Esc = Back)",
        ...
        # real key events: needs the helper started over adb
        self.v_real = tk.BooleanVar(value=cfg["real_keys"])
        ttk.Checkbutton(box, text="Send REAL key and mouse events when the helper is running (real Alt+Tab, "
                                  "real mouse wheel, hover, buttons ... handled by Android itself)",
                        variable=self.v_real).grid(row=3, column=0, sticky="w", padx=6, pady=(6, 3))
        self._bind(self.v_real, "real_keys", bool)
        self.v_rback = tk.BooleanVar(value=cfg["right_click_back"])
        ttk.Checkbutton(box, text="Right-click also sends Back (tick this if right-click does not go "
                                  "Back on your phone in real-mouse mode)",
                        variable=self.v_rback).grid(row=6, column=0, sticky="w", padx=6, pady=(0, 3))
        self._bind(self.v_rback, "right_click_back", bool)
        row = ttk.Frame(box)
        row.grid(row=4, column=0, sticky="w", padx=6, pady=(0, 2))
        self.btn_helper = ttk.Button(row, text="Start real-key helper", command=self._start_helper)
        self.btn_helper.pack(side="left")
        self.lbl_raw = ttk.Label(row, text="", foreground="#555", wraplength=330)
        self.lbl_raw.pack(side="left", padx=8)
        self._raw_msg = ""
        ttk.Label(box, text="Needs adb (Android platform-tools) and USB debugging on. Connect to the "
                            "phone first, then click. The helper keeps running after you unplug the "
                            "cable and stops when the phone reboots.", wraplength=430,
                  foreground="#555").grid(row=5, column=0, sticky="w", padx=6, pady=(0, 4))
    ...
    def _start_helper(self):
        b = self.bridge
        if not b.connected:
            self._raw_msg = "Connect to the phone first."
            return
        self.btn_helper.config(state="disabled")
        self._raw_msg = "Starting the helper over adb..."
        b.raw_info = ""

        def work():
            try:
                token = keyhelper.start_helper(self.cfg["adb_path"], str(self.cfg["usb_serial"]))
                b.send_keysrv(token)
                self._raw_msg = "Helper started, connecting from the phone..."
                end = time.time() + 10
                while time.time() < end and b.connected and not b.raw_keys and not b.raw_info:
                    time.sleep(0.15)
                if b.raw_keys:
                    self._raw_msg = ""                                  # status line shows ON
                elif b.raw_info:
                    self._raw_msg = "The phone could not use the helper: %s" % b.raw_info
                elif not b.connected:
                    self._raw_msg = "Lost the connection to the phone."
                else:
                    self._raw_msg = ("The phone did not answer. Open the MouseShare app on the phone "
                                     "and check the 'Real keys' line.")
            except BridgeError as e:
                self._raw_msg = str(e)
            except Exception as e:
                traceback.print_exc()
                self._raw_msg = "Unexpected error: %s" % e
            finally:
                self.root.after(0, lambda: self.btn_helper.config(state="normal"))
        threading.Thread(target=work, daemon=True).start()
    ...
    def _refresh(self):
        ...
        if b.raw_keys:
            self.lbl_raw.config(text="Real keys ON (%s)" % b.raw_info, foreground="#080")
        else:
            self.lbl_raw.config(text=self._raw_msg or "Real keys off", foreground="#555")
        if self.tray:
