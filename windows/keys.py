...
class KeyRouter:
    ...
    Rules
      ...
      * System-key routing ON : Win tapped alone = Home, Win+D/M = Home, Win+Tab = Recents,
                                Alt+Tab = jump to the previous app, Win+A/N = Notifications,
                                Esc = Back.
      * Everything else is sent as a real key event carrying the held modifiers (KE), except
        plain typing, which is sent as text (KT) so the PC keyboard layout is respected.
    ...
    def _forward(self, vk, scan, mods, repeat=False):
        ...
        if route:
            ...
            if vk == VK_TAB and "alt" in mods:
                if not repeat:                             # holding Tab must not flip back and forth
                    b.send_nav("ALTTAB")
                return True
            if vk == VK_ESCAPE:
                b.send_nav("BACK")
        ...
        if vk in MOD_VKS:
            return True                                    # modifiers only shape other keys
        ...
        if code is not None and not altgr and (vk in SPECIAL_VKS or "ctrl" in mods or "alt" in mods):
            b.send_key_event(code, mask)                   # Ctrl+Backspace, Shift+Left, Ctrl+Z ...
            return True
